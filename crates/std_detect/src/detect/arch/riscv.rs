...
features! {
    ...
    /// columns denote runtime feature detection support with or without
    ...
    /// mean that all performance counters are accessible.
    /// For instance, Linux kernel version 6.6 or later prohibits accesses to
    /// performance counters other than `time` (wall-clock) by default.
    /// Also beware that, even if the performance counters like `cycle` and
    /// `instret` are accessible, their value can be unreliable (e.g. returning
    /// constant value) under certain circumstances.
    ///
